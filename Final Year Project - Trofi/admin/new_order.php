...
<head>
    ...
    if(isset($_GET['packed1'])){
        $id=$_GET['packed1'];
        $status="Packed And Ready to ship";
        $conn = mysqli_connect('localhost','root','','trofi') or die('unable to connect');
        $result_set = mysqli_query($conn,"UPDATE `ingredient_order` SET `status`='$status' WHERE `id`='$id'");
        if($result_set){
            header("location:order.php");
        }
    }elseif (isset($_GET['shiped1'])){
        $id=$_GET['shiped1'];
    ...
    <div class="row">
        ...
        <div class="col-8">
            <div class="card mt-4">
                ...
                <div class="card-body">
                    <table id="table_id1" class="display table">
                        <thead>
                        <tr>
                            <th>Recipe</th>
                            <th>Qty</th>
